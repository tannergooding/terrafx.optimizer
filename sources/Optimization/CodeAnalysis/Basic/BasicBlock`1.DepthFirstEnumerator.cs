...
{
    /// <summary>Enumerates <see cref="BasicBlock{T}"/> instances in depth-first order starting from a root block.</summary>
    public struct DepthFirstEnumerator : IEnumerator, IEnumerator<BasicBlock<T>>
    {
        private readonly BasicBlock<T> _rootBlock;

        private readonly HashSet<BasicBlock<T>> _encounteredBlocks;
        private readonly Stack<BasicBlock<T>> _pendingBlocks;

        private BasicBlock<T>? _currentBlock;

        /// <summary>Creates a new <see cref="DepthFirstEnumerator" /> instance.</summary>
        /// <param name="rootBlock">The root block from which to start the depth-first traversal.</param>
        public DepthFirstEnumerator(BasicBlock<T> rootBlock)
        {
        ...
        public bool MoveNext()
        {
            ...
            if (pendingBlocks.TryPop(out var currentBlock))
            {
